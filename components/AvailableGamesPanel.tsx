...
const AvailableGamesPanel: React.FC<AvailableGamesPanelProps> = ({ games, onPlay, onDelete, onShare, onPrepareWorksheet }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <h2 className="text-xl sm:text-2xl font-bold text-purple-500 dark:text-purple-400 mb-2 sm:mb-4">{t('player.available.title')}</h2>
      {sortedGames.length > 0 ? (
        <ul className="space-y-2 sm:space-y-3 overflow-y-auto pr-1 sm:pr-2 -mr-2 flex-grow">
          {sortedGames.map((game) => (
            <li key={game.id} className="bg-slate-200/50 dark:bg-slate-700/50 p-2 sm:p-3 rounded-lg flex justify-between items-center">
              <div>
              ...
              <div className="flex gap-1 sm:gap-2">
                <button
                  ...
                  className="p-2 sm:p-3 text-green-500 hover:text-green-600 dark:text-green-400 dark:hover:text-green-300 hover:bg-green-500/10 dark:hover:bg-green-900/50 rounded-full transition-colors"
                ...
                >
                  ...
                  <button
                    ...
                    className="p-2 sm:p-3 text-slate-500 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-300 hover:bg-slate-500/10 dark:hover:bg-slate-900/50 rounded-full transition-colors"
                  ...
                  <button
                   ...
                   className="p-1 sm:p-2 text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-500/10 dark:hover:bg-blue-900/50 rounded-full transition-colors relative"
                   title={t('player.available.shareAria')}
                 >
                   <ShareIcon />
                   {copiedId === game.id && (
                     <span className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-xs bg-white dark:bg-slate-900 text-slate-900 dark:text-white px-1 py-0.5 sm:px-2 sm:py-1 rounded shadow-lg border border-slate-200 dark:border-slate-700">
                 ...
                 <button
                   ...
                   className="p-2 sm:p-3 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-500/10 dark:hover:bg-red-900/50 rounded-full transition-colors"
