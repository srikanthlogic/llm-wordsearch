...
const LanguageSelector: React.FC<LanguageSelectorProps> = ({ value, onChange, label, id = 'language-selector' }) => {
    return (
        <div className="animate-fade-in">
            {label && (
                <label htmlFor={id} className="block text-ink text-sm font-semibold mb-2.5">
                    {label}
                </label>
            )}
            <div className="relative">
                <select
                    ...
                    className={`input-base w-full px-4 py-3 appearance-none cursor-pointer transition-all duration-200`}
                    style={{
