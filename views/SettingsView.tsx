...
const SettingsView: React.FC<SettingsViewProps> = ({ aiLogs: _aiLogs, onClearData, theme, onThemeChange, aiSettings, onAISettingsChange, setView }) => {
  ...
  const ThemeButton = ({ current, target, onClick, icon, label }: { current: Theme, target: Theme, onClick: (t: Theme) => void, icon: React.ReactNode, label: string }) => {
    ...
    const activeClasses = "bg-accent/40 border-ink text-ink";
    const inactiveClasses = "bg-sheet border-ink/15 text-ink-soft hover:border-ink/40 hover:bg-ink/5";
  ...
  const BaseField = ({ id, label, children, link }: { id: string, label: string, children: React.ReactNode, link?: string }) => (
    <div className="space-y-2">
        <label htmlFor={id} className="block text-ink text-sm font-semibold">
           <div className="flex items-center justify-between">
            <span>{label}</span>
            {link && <a href={link} target="_blank" rel="noopener noreferrer" className="text-xs text-ink-soft hover:text-ink hover:underline flex items-center gap-1">{t('settings.byollm.docsLink')} <ExternalLinkIcon className="w-3 h-3"/></a>}
           </div>
        </label>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto text-ink space-y-6 overflow-x-hidden animate-fade-in">
        <header className="w-full text-center mb-8">
            ...
            <p className="text-ink-soft mt-2">{t('settings.subtitle')}</p>
        </header>

        <div className="card-elevated p-6 sm:p-8 space-y-6 animate-fade-in-up" style={{ animationDelay: '50ms' }}>
            <div className="flex items-center gap-3 pb-4 border-b border-ink/10">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <button
                  ...
                  className={`p-4 rounded-xl border-2 transition-all duration-200 text-left ${
                    provider === AIProvider.Community
                      ? 'bg-accent/40 border-ink text-ink'
                      : 'border-ink/15 hover:border-ink/40 hover:bg-ink/5'
                  }`}
                >
                    <h3 className="font-bold text-lg text-ink">{t('settings.provider.community.title')}</h3>
                    <p className="text-sm text-ink-soft mt-1">{t('settings.provider.community.description')}</p>
                </button>
                <button
                ...
                >
                    <h3 className="font-bold text-lg text-ink">{t('settings.provider.byollm.title')}</h3>
                    <p className="text-sm text-ink-soft mt-1">{t('settings.provider.byollm.description')}</p>
                </button>
            </div>
            ...
            {provider === AIProvider.Community && (
              <div className="space-y-4 pt-4 border-t border-ink/10">
                <BaseField id="communityModel" label={t('settings.provider.community.modelLabel')}>
                  <select id="communityModel" value={communityModel} onChange={e => setCommunityModel(e.target.value)} className="input-base w-full px-4 py-3  appearance-none">
            ...
            {provider === AIProvider.BYOLLM && (
                <div className="space-y-5 pt-4 border-t border-ink/10">
                    <BaseField id="providerPreset" label={t('settings.byollm.presetLabel')}>
                      <select id="providerPreset" onChange={handlePresetChange} value={currentPreset.name} className="input-base w-full px-4 py-3  appearance-none">
                    ...
                    <BaseField id="apiKey" label={t('settings.byollm.apiKeyLabel')}>
                        <KeyRoundIcon className="pointer-events-none absolute inset-y-0 left-0 h-full w-5 ml-4 text-ink-faint"/>
                        <input id="apiKey" type="password" value={byollmSettings.apiKey} onChange={e => handleByollmChange('apiKey', e.target.value)} placeholder="sk-..." className="input-base w-full pl-10 px-4 py-3  min-h-[48px]"/>
                    </BaseField>

                    <BaseField id="baseURL" label={t('settings.byollm.baseURLabel')} link={currentPreset.website}>
                        <ServerIcon className="pointer-events-none absolute inset-y-0 left-0 h-full w-5 ml-4 text-ink-faint"/>
                        <input id="baseURL" type="text" value={byollmSettings.baseURL} onChange={e => handleByollmChange('baseURL', e.target.value)} placeholder="https://provider.com/api/v1" className="input-base w-full pl-10 px-4 py-3  min-h-[48px]"/>
                    </BaseField>

                    <BaseField id="modelName" label={t('settings.byollm.modelNameLabel')}>
                        <Wand2Icon className="pointer-events-none absolute inset-y-0 left-0 h-full w-5 ml-4 text-ink-faint"/>
                        {is_open_router ? (
                            <select id="modelName" value={byollmSettings.modelName} onChange={e => handleByollmChange('modelName', e.target.value)} disabled={isLoadingModels} className="input-base w-full pl-10 px-4 py-3  appearance-none disabled:opacity-50 min-h-[48px]">
                              ...
                              <input id="modelName" type="text" value={byollmSettings.modelName} onChange={e => handleByollmChange('modelName', e.target.value)} placeholder="provider/model-name" className="input-base w-full pl-10 px-4 py-3  min-h-[48px]"/>
                        )}
                    </BaseField>

                     <div className="flex items-start gap-3 bg-ink/5 p-4 rounded-xl text-sm">
                        <InfoIcon className="w-5 h-5 flex-shrink-0 mt-0.5 text-ink-soft" />
                    ...
                    {testStatus !== 'idle' && (
                        <div className={`flex items-start gap-3 p-4 rounded-xl text-sm ${
                            testStatus === 'success' ? 'bg-success/10 text-success border border-success/30' :
            ...
            <div className="flex justify-end items-center gap-4 pt-4 border-t border-ink/10">
                {provider === AIProvider.BYOLLM && (
                    <button
                        onClick={handleTestConnection}
                        disabled={testStatus === 'testing'}
                        className="btn-secondary flex items-center gap-2 px-6 py-3 font-semibold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed min-h-[48px]"
                    >
                ...
                <button
                    ...
                    className="btn-primary px-6 py-3 font-bold font-display rounded-xl min-h-[48px]"
                >
        ...
        <div className="card-elevated p-6 sm:p-8 space-y-6 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
            <div className="flex items-center gap-3 pb-4 border-b border-ink/10">
                <div className="w-10 h-10 rounded-xl bg-ink/5 border border-ink/10 flex items-center justify-center">
                    <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <button
                ...
                className="btn-secondary flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 font-bold font-display rounded-xl min-h-[48px]"
            >
        ...
        <div className="card-elevated p-6 sm:p-8 space-y-6 animate-fade-in-up" style={{ animationDelay: '150ms' }}>
            <div className="flex items-center gap-3 pb-4 border-b border-ink/10">
                <div className="w-10 h-10 rounded-xl bg-ink/5 border border-ink/10 flex items-center justify-center">
                    <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="card-elevated p-6 sm:p-8 space-y-6 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
            <div className="flex items-center gap-3 pb-4 border-b border-ink/10">
                <div className="w-10 h-10 rounded-xl bg-ink/5 border border-ink/10 flex items-center justify-center">
                    <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="card-elevated p-6 sm:p-8 space-y-6 animate-fade-in-up" style={{ animationDelay: '250ms' }}>
            <div className="flex items-center gap-3 pb-4 border-b border-ink/10">
                <div className="w-10 h-10 rounded-xl bg-ink/5 border border-ink/10 flex items-center justify-center">
                    <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <button
          type="button"
          onClick={onClearData}
          className="flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3 bg-error/10 text-error hover:bg-error/20 font-semibold rounded-xl transition-all duration-200 min-h-[48px]"
        ...
        <div className="pt-4 border-t border-ink/10">
          <a
            href="#privacy"
            className="flex items-center gap-2 text-ink font-medium underline decoration-accent decoration-2 underline-offset-2 hover:decoration-ink"
          >
