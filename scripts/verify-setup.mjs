...
function checkBrunoCollection() {
  const brunoConfigPath = path.join(__dirname, '../bruno/bruno.json');
...
function main() {
 log('Starting CI/CD setup verification...');
  ...
  if (checkFileExists(path.join(__dirname, '../docs/ci-cd-setup.md'))) {
    const docContent = fs.readFileSync(path.join(__dirname, '../docs/ci-cd-setup.md'), 'utf8');
  ...
  if (checkFileExists(path.join(__dirname, '../environments/README.md')) && 
      checkFileExists(path.join(__dirname, '../environments/.env.example'))) {
